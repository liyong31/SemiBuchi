...
echo "$column" > result-diff.csv
for case in all_ats_pairs/*.ats
do
    # l
	command="timeout $time java -jar SemiBuchi-0.0.1.jar -diff -tarjan $case -set 3 >> result-diff.csv";
	echo $command
	eval $command
	# lazy s
	command="timeout $time java -jar SemiBuchi-0.0.1.jar -diff -lazys -tarjan $case -set 3 >> result-diff.csv";
	echo $command
	eval $command
	# lazy b
	command="timeout $time java -jar SemiBuchi-0.0.1.jar -diff -lazyb -tarjan $case -set 3 >> result-diff.csv";
	echo $command
	eval $command
	# lazy b + lazy s
	command="timeout $time java -jar SemiBuchi-0.0.1.jar -diff -lazys -lazyb -tarjan $case -set 3 >> result-diff.csv";
	echo $command
	eval $command
	# optimized NCSB

...
do
    ...
    echo "$NUM"
	command="timeout $time java -jar SemiBuchi-0.0.1.jar -complement easy$NUM.ba $case >> result-complement-easy.csv";
	echo $command
	eval $command
	# optimized
	command="timeout $time java -jar SemiBuchi-0.0.1.jar -opt -complement easy$NUM-opt.ba $case >> result-complement-easy.csv";
	echo $command
	eval $command
done
